...
OBJS = $(patsubst $(SRC_DIR)/%.c, $(BUILD_DIR)/$(OBJECTS_DIR)/%.o, $(SRCS))

OBJS_DEBUG = $(patsubst $(SRC_DIR)/%.c, $(DEBUG_BUILD_DIR)/$(OBJECTS_DIR)/%.o, $(SRCS))
...
$(DEBUG_BUILD_DIR)/$(OBJECTS_DIR)/%.o: $(SRC_DIR)/%.c $(INCLUDES)
	@mkdir -p $(dir $@)
